...
def main_ciclo():
    ...
    c1 = CameraInfo("192.168.0.12",0)
    c2 = CameraInfo("192.168.0.13",1)
    t_live = threading.Thread(target=readLive,args=([c1,c2],)).start()
    t_receiveImg = threading.Thread(target=ciclo_reconect, args=([c1,c2],)).start()
    c1_data = ModbusHandler("192.168.0.12")
    c2_data = ModbusHandler("192.168.0.13")
    ...
    t_banco = threading.Thread(target=banco_loop, args=([c1_data,c2_data],)).start()
    while not fim_Programa.is_set():
        cmd = input()
        if cmd == "end":
            fim_Programa.set()
            asyncore.close_all()
        elif cmd.split(' ')[0] == "live":
            try:
                ip = str(cmd.split(' ')[1])
                readLive(ip)
            except:
        ...
        elif cmd.find("readcam")>-1:
            print(f"camera 1: apr: {c1_data.aprovados}, rep: {c1_data.reprovados}")
            print(f"camera 2: apr: {c2_data.aprovados}, rep: {c2_data.reprovados}")
        else:
