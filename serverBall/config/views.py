...
class CameraJson(View):
    def get(self,request,valor):
        ...
        if UserPermission(request,nivel_min=2):
            ret = f'{{\"tipo\":\"{dados.tipo}\",\"ip\":\"{dados.ip}\",\"img\":\"{dados.img}\",\"faltantes\":{dados.faltantes},\"total\":{dados.garrafas},\"aprovado\":{dados.aprovado},\"reprovado\":{dados.reprovado}}}'
...
class CamerasJson(View):
    def get(self,request):
        ...
        if UserPermission(request,nivel_min=2):
            for c in dados:
                ret = f'{ret}{{\"tipo\":\"{c.tipo}\",\"id\":{c.id},\"status\":\"{c.status}\",\"img\":\"{c.img}\",\"faltantes\":{c.faltantes},\"total\":{c.garrafas},\"aprovado\":{c.aprovado},\"reprovado\":{c.reprovado}}},'
            ret = ret[:len(ret)-1]+']'
            return HttpResponse(ret)
        else:
            return HttpResponse('falha')
